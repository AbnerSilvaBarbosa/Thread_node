...
const interval = setInterval(() => {
    ...
    if (currentThreads == nodejsThreadCount)
        return;
...
await Promise.all(Array.from({ length: 1000 }).map(() => createThread({ from: 0, to: 1e6 }))).then(result => console.log(result));
clearInterval(interval);
//# sourceMappingURL=erick_treads.js.map